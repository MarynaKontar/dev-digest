 ...
 *
 * Source of truth: docs/repo-intel-plan.md §8. Adapted to real code:
 *   - `repos.id` is a `uuid`, so every `repoId` here is a `string`.
 *   - facade-level rows (SymbolRow / SignatureRow / RefRow) mirror plan §8.
 ...
 *
 * DEGRADED CONTRACT (lead decision — resolves the §8 vs §12 ambiguity):
...
export interface IndexState extends IndexResult {
  ...
  updatedAt: Date;
  /** True when the layer is running on the ripgrep fallback (see §12). */
  degraded?: boolean;
 ...
 * CI may parse diff-scoped on the hot path (plan §17). Indexing runs through
